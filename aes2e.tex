...
% Page heads
\markboth{Hüttemann}{Transferability of counterfactual explanations on models of different architecture}


% Title portion
\title{Transferability of counterfactual explanations on models of different architecture}

%Author Info.
\authorgroup{
\author{Frederik Hüttemann}
\email{(frederik.huettemann@rub.de)}
\affil{Ruhr-Universität Bochum, Bochum, Deutschland}
}

%Abstract
\abstract{This paper discusses the transferability of counterfactual explanations generated for a Convolutional Neural Network (CNN) onto different CNNs with similar architecture. Neural Networks, in particular CNNs, tend to learn similar features when trained on the same data. A counterfactual explanation for a simple CNN also produces a counterfactual explanation for nearly identical model.
...
Machine learning nowadays has many real-world applications. It is used to systematically do decisions in fields like credit lending 
...
here comes some related work such as the counterfacutal explanations paper

\section{FUNDAMENTALS}
\subsection{Convolutional Neural Networks}
\subsection{Convergent Learning}
For a given dataset, there are many different pre-trained models you can find to apply them on a classification task. Eventhough, they differ in their architecture, some of the extracted features of the models are shared among all. Some features align while other features are not consistently learned. \cite{li2016convergentlearningdifferentneural} \\
Therefore, features can be transferred to different models and different tasks. However, the transferability among models decreases with increasing differents for tasks and model architecture. \cite{yosinski2014transferablefeaturesdeepneural}

\subsection{Counterfactual Explanations}
Counterfactual explanations are a way to represent the reason why a 
